...
var createPanel = function (scene, data) {
    var sizer = scene.rexUI.add.sizer({
        ...
        .add(createHeader(scene, data), // child
    { expand: true })
        .add(createTable(scene, data, 'skills', 1), // child
    { expand: true })
        .add(createTable(scene, data, 'items', 2), // child
    { expand: true });
    return sizer;
};
...
var createTable = function (scene, data, key, rows) {
    ...
    for (var i = 0, cnt = items.length; i < cnt; i++) {
        item = items[i];
        table.add(createIcon(scene, item, iconSize, iconSize), undefined, true, 'top', 0, true);
    }
    return scene.rexUI.add.sizer({
        orientation: 'y',
