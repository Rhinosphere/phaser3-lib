...
var AddAudioSpinner = function (scene, x, y, width, height) {
    return scene.rexSpinner.add.custom({
        ...
        update: function () {
            ...
            var prevValue = this.getData('prevValue');
            ...
            for (var i = 0; i < cnt; i++) {
                let line = shapes[i];
                let from = line.getData('from'), to = line.getData('to'), current = Phaser.Math.Linear(from, to, this.value);
...
var AddBoxSpinner = function (scene, x, y, width, height) {
    return scene.rexSpinner.add.custom({
        ...
        update: function () {
            ...
            var left = centerX - halfWidth, right = centerX + halfWidth, top = centerY - halfWidth, bottom = centerY + halfWidth;
            var border = this.getShape('border');
            var fill = this.getShape('fill');
            border
