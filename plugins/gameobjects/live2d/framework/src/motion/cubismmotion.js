...
export class CubismMotion extends ACubismMotion {
    ...
    _lastWeight; // 最後に設定された重み
    _motionData; // 実際のモーションデータ本体
    _eyeBlinkParameterIds; // 自動まばたきを適用するパラメータIDハンドルのリスト。  モデル（モデルセッティング）とパラメータを対応付ける。
    _lipSyncParameterIds; // リップシンクを適用するパラメータIDハンドルのリスト。  モデル（モデルセッティング）とパラメータを対応付ける。
    _modelCurveIdEyeBlink; // モデルが持つ自動まばたき用パラメータIDのハンドル。  モデルとモーションを対応付ける。
    _modelCurveIdLipSync; // モデルが持つリップシンク用パラメータIDのハンドル。  モデルとモーションを対応付ける。
}
