...
export class CubismClippingManager_WebGL {
    ...
    _maskRenderTexture; // マスク用レンダーテクスチャのアドレス
    _colorBuffer; // マスク用カラーバッファーのアドレス
    _currentFrameNo; // マスクテクスチャに与えるフレーム番号
    _channelColors;
    _maskTexture; // マスク用のテクスチャリソースのリスト
    ...
    _tmpMatrix; // マスク計算用の行列
    _tmpMatrixForMask; // マスク計算用の行列
    _tmpMatrixForDraw; // マスク計算用の行列
    _tmpBoundsOnModel; // マスク配置計算用の矩形
    gl; // WebGLレンダリングコンテキスト
}
/**
...
export class CubismClippingContext {
    ...
    _layoutChannelNo; // RGBAのいずれのチャンネルにこのクリップを配置するか（0:R, 1:G, 2:B, 3:A）
...
export class CubismShader_WebGL {
    ...
    _shaderSets; // ロードしたシェーダープログラムを保持する変数
    gl; // webglコンテキスト
}
/**
...
export class CubismShaderSet {
    shaderProgram; // シェーダープログラムのアドレス
    attributePositionLocation; // シェーダープログラムに渡す変数のアドレス（Position）
    attributeTexCoordLocation; // シェーダープログラムに渡す変数のアドレス（TexCoord）
    uniformMatrixLocation; // シェーダープログラムに渡す変数のアドレス（Matrix）
    uniformClipMatrixLocation; // シェーダープログラムに渡す変数のアドレス（ClipMatrix）
    samplerTexture0Location; // シェーダープログラムに渡す変数のアドレス（Texture0）
    samplerTexture1Location; // シェーダープログラムに渡す変数のアドレス（Texture1）
    uniformBaseColorLocation; // シェーダープログラムに渡す変数のアドレス（BaseColor）
    uniformChannelFlagLocation; // シェーダープログラムに渡す変数のアドレス（ChannelFlag）
}
