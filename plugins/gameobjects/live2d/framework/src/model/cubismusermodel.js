...
export class CubismUserModel {
     ...
     * @return モーションクラス
     */
    loadMotion = (buffer, size, name, onFinishedMotionHandler) => CubismMotion.create(buffer, size, onFinishedMotionHandler);
    /**
     * 表情データの読み込み
    ...
    _moc; // Mocデータ
    _model; // Modelインスタンス
    _motionManager; // モーション管理
    _expressionManager; // 表情管理
    _eyeBlink; // 自動まばたき
    _breath; // 呼吸
    _modelMatrix; // モデル行列
    _pose; // ポーズ管理
    _dragManager; // マウスドラッグ
    _physics; // 物理演算
    _modelUserData; // ユーザーデータ
    _initialized; // 初期化されたかどうか
    _updating; // 更新されたかどうか
    _opacity; // 不透明度
    _lipsync; // リップシンクするかどうか
    _lastLipSyncValue; // 最後のリップシンクの制御地
    _dragX; // マウスドラッグのX位置
    _dragY; // マウスドラッグのY位置
    _accelerationX; // X軸方向の加速度
    _accelerationY; // Y軸方向の加速度
    _accelerationZ; // Z軸方向の加速度
    _debugMode; // デバッグモードかどうか
    _renderer; // レンダラ
}
